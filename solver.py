...
def get_available_moves(block_id: int, board: Board, tried_spaces: list[tuple[int, int]] = []) -> list[Move]:

    block = board.blocks[block_id]
    block_position = block.position

    if (block_id, block_position) in tried_spaces:
        #print(f"We already checked block: {get_char(block_id)} at x: {block_position[0]}, y: {block_position[1]}")
        return []
    ...
    #print(f"block: {get_char(block_id)}")
    ...
    if (block_position[0] + block.width < len(occupied[0]) - 1): # ensure we aren't at the right of the board
        ...
        if not right_occupied:
            ...
            # new_board = board
            # new_board.make_move(move)
            #moves.extend(get_all_possible_moves(new_board, tried_spaces, depth=depth+1))
...
def search_move(board: Board, start_time: float, move: Move = None, depth: int = 0) -> list[Move]:
   ...
   #print(f"depth: {depth}")
    ...
    tried_board_states.append(copy.deepcopy(search_board))
